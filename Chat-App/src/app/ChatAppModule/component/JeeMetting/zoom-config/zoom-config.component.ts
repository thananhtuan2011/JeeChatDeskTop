...
export class ZoomConfigComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<ZoomConfigComponent>,
  ...
  onSumbit() {
	...
	Prepareleave(): any {
		...
		let item = {
			...
			SDKKey :controls['SDKKey'].value,
			SDKSecret : controls['SDKSecret'].value,
	...
	AddItem(item: any) {
		this.dangKyCuocHopService.setUpConfigZoom(item).subscribe(res => {
	...
	huongdan(){
		window.open('http://help.jee.vn/articles/85-cau-hinh-cuoc-hop-truc-tuyen-zoom-meeting', '_blank');
	}
}
